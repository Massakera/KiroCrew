...
|---|---|---|---|---|
| [`session/opened`](#sessionopened) | The ledger was created, or a claim re-attached to it. | live | `gateway` | — |
...
The ledger was created, or this claim re-attached to a conversation already on
disk.
...
| `resumed` | bool | required | `true` when this claim re-attached to an existing ledger. | |
...
| `queued_seq` | string | required | The queue entry's own id. A string, not a ledger `seq`. | |
...
**Invariants** — Argument *content* never reaches the ledger. `args_hash` lets two
...
**Invariants** — Carries **no `ref`**. No subagent path opens a child ledger, so a
...
**Pairing** — None. It is written into the parent's ledger, because the child has
none of its own.
...
measures neither, so a child's cost is not recoverable from the parent's ledger.
...
| `summary/written` | Its coverage field names ledger `seq`s, and the summary path holds nothing that can name one, so the field would have to be fabricated. |
