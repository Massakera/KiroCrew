...
| `display` | Theme, density, language, terminal (font, default shell, command completion toggle), remote editor (open remote files in a local editor over SSH) | `DisplayPanel.tsx` | `handlers/themes.py`, `handlers/core.py` | `GET,POST /api/themes`, `GET,PUT /api/config/theme`, `GET,PATCH /api/config/kirocrew`, `GET /api/dashboard/branding` |
