...
| `AGENT_NAME` | `"kirocrew-knowledge"` | `llm_pool.py` | Agent name the worker passes at session start. On kiro-cli it selects the installed spec; every other harness receives the same name on its own start |
...
Both entity extraction (`EntityExtractor`) and internal-URL fetch (`agent_fetch.fetch_url_content`) acquire workers from a shared `LLMPool` — a provider-agnostic, bounded pool (`DEFAULT_POOL_SIZE` = 3) of **long-lived** ACP workers. `LLMPool.start()` reads `agent.acp_backend` once (the same `resolve_selected_backend` gate a persisted value crosses on load) and hands it to every `AcpWorker`. An absent or unselectable value stays the empty string, which is kiro-cli. A `Worker` ABC has three concrete paths:

- **`AcpClient` harnesses** — `AcpWorker` drives `AGENT_NAME` (`kirocrew-knowledge`) through `AcpClient` when that client can actually spawn the id. That is kiro-cli (the empty string; the agent is installed by `agent.py:_install_knowledge_agent`, model `claude-haiku-4.5`, kirocrew-core tools only — no internal MCP wiring in the OSS fork) and the harnesses with their own arm in `AcpClient._spawn` (claude-agent-acp, pi, opencode, goose, and the dormant deepseek and droid arms).
- **Runtime harnesses (`kas`, `codex`)** — both are in `ACP_BACKENDS_ACP_RUNTIME` and neither has an `AcpClient` spawn arm. Passing the id to `AcpClient` would start kiro-cli while `client.backend` still reported the configured harness, and later capability reads (effort channel included) would trust that label. `AcpWorker` starts these on `AcpRuntime` and talks through `AcpSessionProvider` with `owns_runtime=True`, the same transport `AcpProvider` uses for chat. `send_message` joins that provider's `stream()` text chunks. A `create_session` that fails kills the runtime it just spawned, because nothing else owns the process yet. kiro stays on the `AcpClient` path above: it is in the runtime set and it is also `AcpClient`'s default arm.
...
- **Sweep shielding via `register_protected_pid`** — pool workers are direct ACP sessions, **not** `SessionMap` sessions or warm-pool providers, so the gateway's periodic orphan sweep cannot see them via `_collect_active_pids` and would SIGKILL a *busy* worker mid-task (surfacing as `ACP process exited (code=1)`). After the session is ready, `AcpWorker` registers that worker process's PID in the sweep-protected set via `register_protected_pid` (`session_pid.py`), and unregisters it in `shutdown()` and on respawn. The PID is whichever process the worker started: the `AcpClient` child (kiro-cli, pi-acp, claude-agent-acp, opencode, goose) or the `AcpRuntime` process when the backend is KAS or codex. The orphan sweep then treats it like a live session (the same `register_protected_pid` mechanism the shared `WorkerPool` engine in `acp/worker_pool.py` applies to `ReviewPool`'s `AcpReviewWorker`).
- **`audit_source="subagent"`** — this argument belongs to `AcpClient`. Those workers run tools without passing through `chat_runner` or `SubagentManager`, so their tool calls would otherwise never reach the security audit log. With `audit_source` set, `AcpClient` emits an SEL `log_tool_invocation` record (`source="subagent"`, `outcome="auto_approved"`) per auto-approved tool call; the emit is offloaded to `subprocess_executor()` and bounded by `asyncio.wait_for` so a hung SEL backend never stalls tool dispatch. `None` (chat/subagent clients) never double-logs. A KAS or codex worker has no `AcpClient`; permission decisions on that path are recorded by `AcpSessionHandle` (`source="acp_session_handle"`).
