...
**Scrub before send.** State is matched against the credential patterns in `credential_patterns` before any request is constructed. A hit cancels the call: nothing is sent, `None` is returned, and the log line records `scrubbed`. The refusal is whole-request, not per-field redaction — a partially redacted payload is still a payload derived from a credential, and a judge's answer over redacted state would be logged as if it had judged the real thing.
