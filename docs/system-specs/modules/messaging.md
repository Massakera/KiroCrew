...
shows it only the part of `answered` that bubble was DISPLAYING -- a cap can leave
the rest of its messages queued, and naming those would claim them answered.
`answered` carries one conversation's messages: every drain collapses a turn from
entries sharing a sender and a place and defers the rest, and builds the flip's
surface from that same entry, so a co-tenant's copy is never in it. The session's
other receipts are then reconciled against `answered` off their own `texts` --
exactly what each displayed -- rather than guessed from the channel. One whose every text the turn
...
use it FOR A CO-TENANT: a text the session wants more copies of than were
answered, and that more than one conversation is displaying, is claimed by no
co-tenant, because claiming the copy would spend what another conversation's own
message accounts for and present as answered a message that may still be queued in
another chat. Those
bubbles stay live rather than one being finalized on whichever the dict yielded
first, and an unattributable leftover is not counted as a conversation's own
...
never does. The conversation the flip is KEYED to is outside that test, because
`answered` holds no copy but its own: subjecting it to the test made the
self-correction circular, since the next turn it defers to is the very turn that
would have to claim the copy -- so two ordinary people who both sent "ok" had both
bubbles stranded on "⏳ Queued" for good, each turn declining to attribute the
message it had just answered. Exact per-entry attribution needs the
...
a co-tenant holds a receipt the body states only what the split proved this
conversation did not get answered, so a miscounted total cannot reach another
person's chat as a promise of messages they never sent. Either
...
silence: the conversation that finished the turn has no bubble to flip -- its send
failed, so it was never recorded -- and without its texts no co-tenant can be
reconciled either, so every bubble is left for its own turn to resolve.
