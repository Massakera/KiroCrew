...
19. **Hook auto-approve platform scope**: reports whether a name-based auto-approve can be satisfied on this host, read off the same `name_grant.environment_refusal` helper `name_grant.name_grant_refusal` consults, so the row cannot describe a posture the check does not hold. Two answers, printed with the source explanation because the only other trace is a decline line in `gateway.log` per invocation and what a user would have to read to tell the states apart is the source of `name_grant`: `⚠ declined by this environment (<code>)` when an environment refusal holds — a relative or empty `PATH` entry (`search_path_has_a_relative_entry`), or on POSIX an inherited `BASH_ENV`/exported shell function (`inherited_env_can_redefine_programs`) — printed with the detail naming the state so the user can act on it; and `✅ name grants can be satisfied on this platform` otherwise. Windows prints `✅` exactly like macOS and Linux absent such a refusal: kiro-cli spawns the shell profile-free (Windows PowerShell 5.1, `-NoProfile`, see `name_grant.MODELLED_WINDOWS_SHELL`), so a per-user `$PROFILE` never runs before the command and cannot inject a function ahead of a program, and the name-grant check returns per-command verdicts on Windows the same way it does on POSIX. Never an issue and never part of the exit code: each fail-closed answer is the intended posture. `name_grant.should_log_decline` bounds the human-facing warning on the surfaces that consult it — the agent hook webhook, the dashboard chat runner and `llm_helpers` — collapsing a repeated line for the same session and environment fingerprint to one; the Slack gateway and handler, the messaging driver, `channel`, the subagent runner and the task runner call `name_grant.log_decline` without that gate, so those warn on every decline. `name_grant.log_decline` itself writes the SEL audit row for every decline on every surface, because declining is a security decision and only the human-facing line is ever deduplicated.
