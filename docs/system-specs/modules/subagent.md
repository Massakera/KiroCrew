...
- `backend` (str, optional) / `backends` (list[str], optional, one per task): the ACP harness a subagent runs on, by WIRE name (`kiro`, `codex`, `claude`… — the `POLICY_ID_BY_BACKEND` vocabulary, because the kiro id is `""` and cannot be told from "not given" once stored). `backends` mixes harnesses in one wave; `backend` applies to every task. Vocabulary and refusals live in `kiro_crew/subagent_backend.py`. `/api/spawn` REFUSES, never degrades: a name this deployment cannot select answers `unknown_backend` (with the selectable roster), and a harness whose install probe (`agent_sdk.backend_install.probe_backend`, off-loop) says `MISSING` answers `backend_unavailable` with the missing component and install command; `UNKNOWN` is not a refusal. The tool stops re-posting a backend already refused in the same wave, like an unknown agent. Accepted, the value is `SubagentInfo.acp_backend`, persisted in `state.json`. An explicit per-spawn value is written at folder creation. A spawn that omits `backend` leaves the field empty until the provider starts, so that first start still takes the default path (session sharing and the warm pool included); once the provider exists, the wire name it is actually serving is frozen onto `SubagentInfo.acp_backend` and into `state.json`. Continuation, retry and failover read that frozen name, so a later change to `agent.acp_backend` cannot resume the conversation on a different harness or cool down a backend that never ran. The field is carried through the queue dict and `POST /api/spawn/{id}/retry`, and INHERITED by `spawn_continue` from the conversation's first run — the harness owns the session record a continuation resumes, so it is never re-chosen. At run time a non-empty value forces the dedicated-process path (the parent's shared runtime is a process of the parent's harness), bypasses the warm pool (`pool_decision = "bypass_backend"`), and reaches the provider factory as `acp_backend_override`, which feeds the ONE selection gate (`members.select_provider_backend`) as its highest-precedence input — override, then member-DM route, then `agent.acp_backend` — through the same `resolve_selected_backend` governance gate. A solo spawn naming another backend than the parent's live harness passes the solo gate on the ground `backend`. The effective backend is echoed on the `/api/spawn` receipt, `GET /api/spawn[/{id}]`, `spawn_list`, the subagent WS frame, a `Backend:` header line on the completion event, and `backend` in its structured meta.
- A **pi** parent calls these tools through Crew's pi tool bridge, not the MCP array pi-acp ignores: with `mcp_gateway.stub_servers` naming `kirocrew-core`, the session carries `spawn_run`, `spawn_continue`, `spawn_steer`, `spawn_release`, `spawn_list`, `spawn_status` and `spawn_sub_agents` as `mcp__kirocrew-core__<tool>`, each call gated like any other and bound to the parent session by the broker stub's token (see acp-client.md § Pi tool bridge). The children themselves run on whatever backend `backend` / `backends` names.

- Rate limits across backends (`kiro_crew/subagent_backend.py`). Backoff WITHIN a backend is upstream's and runs first: the in-turn transient ladder, and the dependency coordinator's schedule for a typed provider throttle. A run they gave up on, whose error reads as a rate or usage limit (`is_rate_limited_error`) and which ran NO tool (`tool_count == 0`, so a replay cannot repeat a side effect), FAILS OVER: its effective backend cools down (`EXHAUSTED_COOLDOWN_SECS` for a spent window, `RATE_LIMITED_COOLDOWN_SECS` for a burst), `pick_fallback` walks `agent.subagent_backend_fallback` past it (skipping cooling, unselectable and uninstalled backends), and the same task is re-dispatched through `spawn_async` (so a durable task-queue accept stays on the writer thread) with the run's scope. The failed run's error then names the replacement id, so the parent waits for one more completion instead of retrying. Continuations, `keep` runs and wave members are not failed over (their conversation or accounting belongs to the original). Decided once per run (`_failover_checked`), from `run.py`'s failure arm before `done` flips and again, as a no-op, at the terminal report. `agent.subagent_backend_limits` caps running sub-agents per effective backend: the admission gate queues a spawn whose backend is full with no slot free for it, and `pick_window_index` skips such entries so an uncapped one behind it still starts. Both manager hooks are looked up on the manager's CLASS so a manager double reports as before.
...
- Only a NON-timeout failure of the shared runtime itself (dead, spawn failed)
  ...
  explicit `model` / `reasoning_effort` / `acp_backend` / `allowed_tools` / `bare` /
  private memory constraints take the dedicated path by decision, not by fallback.
