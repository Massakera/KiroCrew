...
| `steer` / `supports_steer` / `last_steer_monotonic` / `supports_steering_extension` / `inject_steering` | The steer extensions. `supports_steer` is `_session/steer` (the chat echo path). `supports_steering_extension` and `inject_steering` are `_session/steering` for subagent steer; both default off (harness-parity H14). `injected` and `startedNewTurn` count as delivery; any other outcome does not. Non-implementers of `_session/steer` answer `-32601`, so `supports_steer` must be honest. |
