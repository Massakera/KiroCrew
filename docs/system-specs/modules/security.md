...
- `inline_payload.py` — what an INLINE interpreter program names: the credential-mint gate for `python -c` and the stdin forms, asked of the payload text alone. The mint surface (`_MINT_SURFACE_RE`), the two static resolutions that read a hidden name back out (`_fold_inline_literals` for `'a' + 'b'` pieces, `_decoded_b64_literals` for base64 literals of the command as submitted), and the one predicate the floor calls (`_inline_payload_reaches_cli`). Imports only the vocabulary; the floor decides which tokens are a payload, this module decides what the payload names.
