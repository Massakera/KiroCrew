...
rewrite. The gateway therefore keeps its own record of the kinds that were approved, in the app's
`installed.json` under `approvedUnits`, and a request is granted a kind only when the manifest
still declares it AND that record already holds it.

That record is written at install and at update, from the manifest the operator installed. An
update may widen it; an update that fails leaves it exactly as it was. A self-registration through
the App Kit API runs under the app's own token, so it may only narrow the record — dropping a kind
it no longer wants — never add one.
...
app installed before `approvedUnits` existed reads as approving nothing, so it is in exactly this
state until it is updated.
