...
| [contribution-protocol.md](contribution-protocol.md) | How an out-of-process contributor reads a unit's log, appends under its own namespace, and publishes projected views the gateway serves as its own: the manifest grant, the REST and WebSocket surface, and the store's acceptance rules. |
