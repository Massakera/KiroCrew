...
true against a stale mirror and the version badge describes a tip that has already
moved. With no in-place advance, the row's report IS the whole product of
...
it publishes (`git ls-remote --tags <remote> refs/tags/<tag> refs/tags/<tag>^{}`) and refuses unless the resolved
...
names only the resolved tag's exact ref **and** its explicit `^{}` pattern, so an annotated
release yields both its tag object and commit without enumerating the remote's tag population;
parsing accepts only those two refs and retains at most their two bounded oids. An unadvertised
tag is **not** a git error (exit 0, no output), so absence is read from the result rather than
from the exit code. A failed query refuses, and an unadvertised tag
...
channel tip, so a stale row can name both. Publishing the resolved version as an adopted
row's `version` made the badge rename itself to every new release as it shipped while
the checkout stayed put — the row claimed a build it did not contain. `version` is
`null` when the tree is detached at no release tag at all
...
`worktree` is non-null only for an adopted tree. The row's currency is the badge:
`version` names the release the tree contains and `at_tip` selects its ok/warn variant
relative to the **channel tip**, not `BASE_BRANCH`. The two are answered independently:
`version` is read from the local mirror for the tree's own HEAD, so a failed tip lookup
leaves `version` in place (the tree did not change because the channel could not be read)
and sets `at_tip` to `null`, which the badge renders as the release the tree holds with a
tooltip saying the tip could not be resolved — never as "not on a release". The frontend
renders BEHIND and PR as *inapplicable* for this detached release pin.
...
Create first runs `worktree add --no-checkout --detach` at the unique staging path, then
probes from that staging checkout, and only if the probe is clean populates it with `reset
--hard <oid>`. The add and reset run at the **`strict` git tier**. Population can run
whatever `filter.<name>.process`/`.smudge`/`.clean` driver applies to the files it writes --
an arbitrary command -- and `<name>` is attacker-chosen, so the fixed git env neutralizers
(hooks, fsmonitor, credential helper, `sshCommand`) cannot cover the key space. Refusal is
the primary control; `strict` is the second layer, injecting no trusted credential helper
and masking every credential home if a driver reaches execution anyway.

The probe MUST run from the staging checkout because `includeIf gitdir:` is evaluated
against the gitdir of the worktree being operated on. A conditional include scoped to
`.../.git/worktrees/release-channel-stable.staging.*` is invisible from the primary
checkout, but visible from the linked staging worktree whose populate it guards.
`--no-checkout` is what makes that exact gitdir context available before any
repo-controlled file is written.

A driver is only ever defined in a config FILE. `.gitattributes` can name a filter for a
path but cannot say what it runs, and a fetch carries refs and objects, never config -- so
the release cannot introduce a driver. The staging probe reads the repository's local
config plus that linked worktree's `config.worktree` when `extensions.worktreeConfig` makes
the scope live. Both use `--includes`; global and system config are deliberately excluded
because they belong to the operator's machine. A scope that will not answer fails CLOSED.
The add, probe, and populate stay under `_GIT_MUTATION_LOCK`, with no network call between
the probe and populate. Config remains writable by another process, so the sub-millisecond
gap between those two local spawns is the residual; the strict tier is the second layer.

The add and populate are also **drained across cancellation** (`_run_uninterruptible`,
the same treatment the destructive removal path gets), and take `_GIT_MUTATION_LOCK`
inside the per-worktree lock, which is the order the section header declares. A plain
`await` can release the lock while git is still registering the linked worktree or writing
its files. Reads that mutate nothing remain interruptible.

An add that fails **after** registering, a staging-context filter refusal, or a populate
failure all remove and prune the same unique **staging path** (`<worktree
path>.staging.<pid>.<rand>`), deleting both its directory and admin record. Only a clean,
fully populated detached tree is adopted with `git worktree move`.
...
reading the same two repository-supplied scopes with the same mandatory `--includes` and
the same fail-closed rule. Create supplies the staging checkout so conditional includes
are evaluated in the same gitdir context as its populate --
`release_channel_pin.repo_supplied_filter`, mirroring
...
**The version badge is the currency signal and names no control.** `version` states what
the tree contains and `at_tip` changes the badge from ok to warn when the channel has
published a newer release. The badge does not name the action that closes the gap: that is
Remove followed by Create, two controls on two surfaces (Remove in the row's detail panel,
Create on the placeholder that appears afterwards), and naming one would misdirect.
