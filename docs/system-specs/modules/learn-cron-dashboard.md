...
are refused until armed again) to name this loop id on this slot. The authorizer mints the loop id itself, writes that record BEFORE
...
Schedules pane) arms the member's own thread from OUTSIDE its turn, which the self-arm exception does not cover, so
...
`monitor_update` and the fire path go through the nudge service's locks, so
the takeover's rollback still targets exactly the loop id and token it
captured. ON on a STOPPED
...
(best effort, the record kept for its words), and a CANCELLED initial arm
forgets the entry it wrote before the add committed (joined), so no entry
stands over a loop that is paused or never existed; a structured monitor is refused 409, never converted. Inside
...
the member itself" plus that text, and the owner's switch on the detail page
