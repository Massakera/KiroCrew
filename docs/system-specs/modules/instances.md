...
record may never have passed through the API. At the same boundary, loopback
requires the stored `remote_port` to be in `1..65535`, just as SSH and SSM
validate their transport-specific fields, before any socket dial can use it.
`diagnose()` runs the same resolver before the loopback probe and returns
...
- **Self-heal retains the pane listener.** A failed destination generation
  ...
  Probe-failure condemnation remains recoverable. Revoking
  `instances.allow_loopback_transport` is terminal for each live tunnel: the
  manager raises its reconfiguration barrier, cancels and awaits in-flight
  recovery, and only then tears the tunnel down; `rearm_loopback()` cannot clear
  that revoked verdict. Explicit disconnect and failed initial readiness still
  release the listener. This keeps a mounted iframe's host-scoped cookie from
  reconnecting to a local process that won a transient port-release race.
