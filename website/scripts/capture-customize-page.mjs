...
try {
  for (const theme of ['dark', 'light']) {
    ...
    // ── Default crewmate: the server refuses the pick ─────────────────
    {
      ...
      // Our sentence first, the server's reason after it.
      if (!/^Could not change the default crewmate — unknown agent: reviewer/.test(((await notice.textContent()) || '').trim())) throw new Error(`${theme} default-crewmate-refused: notice reads "${await notice.textContent()}"`)
