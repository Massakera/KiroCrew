 ...
 *   4-not-found.png      the command changed to one the machine lacks and saved:
 *                        "Saved. The command was not found on this machine.",
 *                        the row Missing with the command named and the remedy
 *   form-in-use.png      Use Custom ACP pressed: the row reads In use
 *   form-in-use-remove.png  Remove pressed while in use: the question names where
 *                        new sessions go
 *   5-confirm.png        Remove Custom ACP pressed: the question and its Keep it /
 *                        Yes, remove it answers, nothing written yet
 *   6-removed.png        removal confirmed: fields emptied, "Removed." line
 *
 * There is no separate "configured" still: the post-save frame IS the configured
 * scene, and a second capture of the same state was read as an accidental
 * duplicate.
 *
...
// A held PATCH is what gives "Saving…" a frame to be caught in; `absent` names the
// command the not-found frame saves.
const ABSENT = 'my-acp-not-installed'
await p.goto(
  `${base}/capture/custom-acp-form.html?scene=empty&theme=dark&patch_delay_ms=2500&absent=${ABSENT}`,
...
await shot('3-saved')
// The configured scene's descriptor sentence is asserted here, on the frame that
// already shows it, rather than captured a second time.
await strip.getByText(/^The ACP agent you set up below\./).waitFor({ timeout: 20_000 })

// The other answer the re-check can give: the command was written but the machine
// has no such program. The status line says so, the row reads Missing with the
// command named, and the remedy points back at the field -- the frame the string
// `custom_saved_missing` had never had.
const command = form.getByLabel(/^Command/)
await command.fill(ABSENT)
await save.click()
await p.getByText('Saved. The command was not found on this machine.').waitFor({ timeout: 20_000 })
if (!/Missing/.test((await customTab.textContent()) ?? '')) {
  throw new Error('the row does not read "Missing" for an absent command')
}
await strip.getByText(ABSENT, { exact: false }).first().waitFor({ timeout: 20_000 })
// The remedy is the dashboard's own sentence naming the Command field one screen
// down -- not the gateway's `config.json` wording behind a Copy button.
await strip
  .getByText(`Install whatever provides ${ABSENT}, or correct the Command in the form below.`)
  .waitFor({ timeout: 20_000 })
if ((await strip.getByRole('button', { name: 'Copy command' }).count()) !== 0) {
  throw new Error('the custom Missing panel still offers a Copy command button')
}
await shot('4-not-found')
// Back to the command the machine has, so the frames below show a row that runs.
await command.fill('my-acp')
await save.click()
await p.getByText('Saved. The command was found.').waitFor({ timeout: 20_000 })
