    ...
    >
      ...
      {meetCrewmates.eligibilityError && !meetCrewmates.open && (
        /* The Meet CrewMates eligibility read failed, so the chapter cannot
           decide whether to fire. Said here rather than swallowed; the agent
           hand-off is safe because nothing is being typed. */
        <div className="fixed bottom-safe-offset-4 right-safe-offset-4 z-40 max-w-sm flex flex-col gap-2" data-testid="meet-crewmates-eligibility-error">
          <ErrorNotice
            askAgent
            message={i18nT('components.meetCrewmatesFlow.eligibility_error')}
            onHandoff={meetCrewmates.dismissEligibilityError}
          />
          <button
            type="button"
            className="self-end px-3 py-1 rounded-lg text-[12px] font-medium cursor-pointer bg-card border border-border text-text hover:border-border-strong transition-colors"
            onClick={meetCrewmates.dismissEligibilityError}
          >
            {i18nT('app.dismiss')}
          </button>
        </div>
