...
// grant's real scope (agent-scoped, until restart; blanket = channel-wide,
// persisted).
...
const simpleMeta = {
  request_id: '7', tool_title: 'Running: ls -la /tmp', tool_input: '{"command": "ls -la /tmp"}',
...
describe('ChannelPage approval card with meta', () => {
  it('hides the base tier for a compound command the server cannot grant a base for', async () => {
    ...
    expect(items[0]).toBe('Trust “cat f | wc -l” for @Researcher — until restart')
  ...
  it('labels the base tier with the server-derived binary and grants exactly that', async () => {
    ...
    expect(items.map(i => i.textContent)).toEqual([
      'Trust “ls -la /tmp” for @Researcher — until restart',
      'Trust all ls commands for @Researcher — until restart',
      BLANKET,
    ])
    fireEvent.click(items[1])
    await waitFor(() => expect(vi.mocked(api).channelApproveAgent)
      .toHaveBeenCalledWith('ch1', 'a1', 'trust_base', 'ls *'))
    expect(screen.getByText('Trusted — ls commands are auto-approved for @Researcher until restart')).toBeInTheDocument()
  })
  ...
  it('prefers the meta title over the prose for the exact-command grant', async () => {
    ...
    expect(screen.getByText('Trusted — “ls -la /tmp” is auto-approved for @Researcher until restart')).toBeInTheDocument()
  })
