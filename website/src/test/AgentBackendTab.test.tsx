...
describe('AgentBackendTab custom harness', () => {
  ...
  it('saves the record whole, then refreshes and re-checks the row', async () => {
    ...
    // The first sentence -- what is and is not checked -- is the strong text in
    // the paragraph; the rest stays fine print. A skimming reader stops on the
    // claim the deny rules depend on instead of filing the whole block as legal.
    const lead = form.getByTestId('custom-acp-attestation').querySelector('.text-text-strong')
    expect(lead).toHaveTextContent(
      /^Kiro Crew checks that the agent offers this option and turns it on, but it cannot check what the option means\.$/,
    )
    ...
    // A glyph in front marks the line as a message: beside the Save button, a bare
    // "Saved." read as a second button. Decorative -- role="status" carries the
    // meaning -- so it is hidden from assistive tech.
    expect(form.getByRole('status').querySelector('svg[aria-hidden="true"]')).not.toBeNull()
  ...
  it('points a missing custom command at the form below, not at config.json behind a Copy button', async () => {
    kirocrewConfigMock.mockResolvedValue({
      agent: {
        acp_backend: '',
    ...
    acpBackendsMock.mockResolvedValue({
      backends: [
        probeRow('', card()),
        customRow({
          // Registered, so selectable like any missing built-in; only the binary is absent.
          selectable: true,
          installed: 'missing',
          missing_components: ['my-acp'],
          install_command: "install whatever provides 'my-acp', or correct agent.custom_acp.command in config.json",
        }),
      ],
    })
    wrap()
    ...
    highlight('Custom ACP')
    await waitFor(() =>
      expect(panel().getByText('Missing on this machine: my-acp')).toBeInTheDocument(),
    )
    // The remedy names the field that edits the value, one screen down, in prose.
    expect(
      panel().getByText('Install whatever provides my-acp, or correct the Command in the form below.'),
    ).toBeInTheDocument()
    // The gateway's config.json sentence is the CLI's remedy; it is not shown here,
    // and there is no Copy button offering to copy a sentence.
    expect(panel().queryByText(/agent\.custom_acp\.command in config\.json/)).toBeNull()
    expect(screen.queryByRole('button', { name: 'Copy command' })).toBeNull()
    // The field the remedy names is on the page.
    expect(screen.getByRole('form', { name: 'Custom ACP agent' })).toBeInTheDocument()
  })
