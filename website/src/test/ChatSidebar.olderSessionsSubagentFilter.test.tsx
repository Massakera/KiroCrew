 ...
 *
 * Pins: subagent rows (backend is_subagent flag OR a subagent:/subagent_/_bg
...
const SUB_FLAG = { key: 'chat-3-300', title: 'flagged subagent run', messages: 1, modified: 2800, is_subagent: true }
...
describe('Older Sessions — sub-agent filtering', () => {
  it('hides sub-agent sessions by default while showing normal chats', () => {
    ...
    // Both a backend-flagged and a key-prefixed subagent row are excluded.
    expect(screen.queryByText('flagged subagent run')).toBeNull()
  ...
  it('reveals sub-agent sessions when the toggle is switched on, and persists the choice', () => {
    ...
    expect(screen.getByText('flagged subagent run')).toBeInTheDocument()
