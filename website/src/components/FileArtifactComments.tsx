...
export function useFileArtifactComments({
  ...
  iframeSelection: { text: string; x: number; y: number } | null
  toggleSidebar: () => void
  sidebarOpen: boolean
  commentCount: number
  ...
  const [iframeSelection, setIframeSelection] = useState<{ text: string; x: number; y: number } | null>(null)
  ...
  const onIframeSelect = useCallback((sel: { x: number; y: number; quote: string; prefix?: string; suffix?: string }) => {
    ...
    setIframeSelection({ text: sel.quote, x: sel.x, y: sel.y })
  }, [])
  ...
  // Whether the open box holds unsaved text (mirrored from the toolbar).
  const composerDraftRef = useRef(false)
  const handleComposerDraftChange = useCallback((hasDraft: boolean) => { composerDraftRef.current = hasDraft }, [])
  ...
  }), [handleComposerOpen, addAnchored, handleComposerClose, handleComposerDraftChange, confirmDiscardDraft])
  ...
  return {
    overlay, popovers, sidebar, selectionComposer, hasComposerDraft, iframeSelection, toggleSidebar, sidebarOpen,
