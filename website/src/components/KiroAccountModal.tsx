...
function CreditUsage({ usage, onClose }: { usage: KiroAccountUsage; onClose: () => void }) {
  ...
  const refreshButton = (compact: boolean) => (
    <RefreshButton
      ...
      held={refresh.held}
      onClick={() => refresh.mutation.mutate()}
      compact={compact}
    />
  )
  ...
  if (usage === 'failed') {
    // One notice at a time: a refresh outcome REPLACES the standing "could not
    // read" notice instead of stacking under it, so the box never carries two
    // alerts. The standing notice returns once the outcome clears.
    ...
    return (
      <div className="flex flex-col gap-3">
        {outcomeReplacesNotice
          ...
          : (
            <ErrorNotice
              message={i18nT('components.kiroAccountModal.credit_usage_unavailable')}
              askAgent
              onHandoff={onClose}
            />
          )}
