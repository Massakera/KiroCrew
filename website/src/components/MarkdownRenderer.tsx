...
function RemoteHostFact({ remotes, className }: { remotes: readonly string[]; className?: string }) {
...
function ImgWithFallback({
  ...
  return (
    ...
    <span className="relative block my-2" ref={approvedImageRef} tabIndex={-1}>
      ...
      {isRemoteMediaUrl(url) && <RemoteHostFact remotes={[url]} className="mt-1 block text-[11px] leading-relaxed text-muted" />}
    </span>
  )
}
...
function DeferredMedia({ tag, node, children }: { tag: 'video' | 'audio'; node?: HastElement; children?: React.ReactNode }) {
  ...
  return (
    <MediaApprovedCtx.Provider value={true}>
      ...
      <RemoteHostFact remotes={remotes} className="mt-1 block text-[11px] leading-relaxed text-muted" />
