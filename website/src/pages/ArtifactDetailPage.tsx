...
export default function ArtifactDetailPage({ popout = false }: { popout?: boolean } = {}) {
  ...
  /** Run `proceed` unless an unsaved comment draft would be lost, in which case ask first. */
  const guardCommentDraft = useCallback(async (proceed: () => void) => {
    if (composerDraftRef.current && !(await confirmDiscardDraft())) return
    proceed()
  }, [confirmDiscardDraft])
  ...
  }), [handleComposerOpen, addComment, handleComposerClose, handleComposerDraftChange, confirmDiscardDraft])
