...
function ChangeDefaultLink() {
  return (
    <Link
      ...
      title={i18nT('pages.kiroCrewAgentsPage.default_badge_hint')}
      className="inline-flex items-center gap-0.5 rounded text-[12.5px] text-accent hover:underline focus-ring"
    >
      {i18nT('pages.kiroCrewAgentsPage.change_default_crewmate')}
...
function CrewSourceBadge({ source }: { source: string }) {
  const label = source === 'user' ? i18nT('lib.templateSource.custom')
