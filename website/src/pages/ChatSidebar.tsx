...
interface HistoryItem {
  ...
  is_subagent?: boolean  // sub-agent/sub-task session (key prefixed subagent:/subagent_, or _bg); hidden by default
}
 ...
 *  (key-prefixed subagent:/subagent_, flagged is_subagent by the backend).
...
function useDebouncedSessionSearch<T>(
  query: string,
  transform: (sessions: { key: string; title?: string; created?: string; modified?: number; agent?: string; memory_mode?: 'persistent' | 'incognito' | 'temporary'; folder_id?: string; tags?: string[]; is_subagent?: boolean; instance_id?: string; instance_name?: string }[]) => T,
 ...
 *  `is_subagent` flag when present, and falls back to the same key-prefix
 *  classification the backend uses (subagent:/subagent_) so search results or
 *  older cached rows lacking the flag are still recognised. The `_bg`
 *  background session is deliberately NOT treated as a sub-agent — the "Show
 *  sub-agent sessions" toggle label would not describe it. */
function isHistorySubagent(item: { key: string; is_subagent?: boolean }): boolean {
  if (item.is_subagent) return true
  const k = item.key
            ...
            <div className="overflow-y-auto scrollbar-none p-2 scroll-shadow" style={{ height: `${historyHeight}px`, scrollbarWidth: 'none' }}>
              {(() => {
                ...
                const filteredHistory = (() => {
                  ...
                  return hideSubagents ? base.filter(s => !isHistorySubagent(s)) : base
                })()
                // How many rows the sub-agent toggle removed from THIS view. When
                // it accounts for the whole visible list being empty (a page that
                // is all sub-agent rows), the empty state says so and points at the
                // toggle instead of reading as "no sessions".
                const hiddenSubagentCount = hideSubagents
                  ? (historyFilter
                      ? (historySearchResults ?? history).filter(s => historyLocalMatch(s) && isHistorySubagent(s)).length
                      : history.filter(isHistorySubagent).length)
                  : 0
