...
export default function DevFleetPage() {
  ...
  // One value, not a per-lane map: the backend publishes exactly one
  // `release_channel` (its `lane` is a constant), so there is one placeholder row
  // and one Create to fail. A map keyed by lane generalised over a set of one.
  const [releaseChannelCreateError, setReleaseChannelCreateError] = useState<string | null>(null)
  ...
  async function createReleaseChannel() {
    ...
    setReleaseChannelCreateError(null)
    try {
      ...
      else setReleaseChannelCreateError(r?.error ? `${i18nT('pages.devFleetPage.release_channel_create_failed')}: ${r.error}` : i18nT('pages.devFleetPage.release_channel_create_failed'))
      invalidateFleet()
    } catch (e: unknown) { setReleaseChannelCreateError((e as Error)?.message || String(e)) }
  ...
  function renderChannelPlaceholder(rc: ReleaseChannel) {
    ...
    // dismissed it. Held outside the fleet payload so the refetch that follows a
    // failed Create cannot clear it.
    const createErr = releaseChannelCreateError
    return (
      <div key={'rc-' + rc.lane}>
      <div data-testid={'release-channel-placeholder-' + rc.lane} style={{ display: 'grid', gridTemplateColumns: '16px 84px minmax(0,1fr) 64px 48px 44px 212px', gap: 8, alignItems: 'center', padding: '5px 0', borderTop: '1px solid var(--border)', minHeight: 30, minWidth: 640 } as CSSProperties}>
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center', minWidth: 0 } as CSSProperties}>
          ...
          <Btn onClick={() => createReleaseChannel()} disabled={rcBusy || !!rc.error} title={createTitle}>
      ...
      {createErr ? (
        <div style={{ margin: '2px 0 8px 32px' }}>
          <ErrorNotice message={createErr} askAgent onDismiss={() => setReleaseChannelCreateError(null)} testId={`release-channel-create-error-${rc.lane}`} />
        </div>
      ) : null}
      </div>
    )
