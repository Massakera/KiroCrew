        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(8rem,13rem)_minmax(0,1fr)]">
          ...
          >
            ...
            >
              ...
              {status(shown)}
              {/* The custom harness has no installer Crew can name, and its command is
                  edited in the form directly below this strip -- so the remedy points
                  there, in prose, with no Copy button: the gateway's sentence names
                  `agent.custom_acp.command in config.json`, which is the right remedy
                  for the CLI and a detour here, where the field that edits exactly
                  that value is one screen down. */}
              {shown === CUSTOM && notInstalled(shown) && !unconfigured(shown) && (
                <p className="mt-1.5 mb-0">
                  {i18nT('pages.developer.agentBackendTab.custom_missing_remedy', {
                    command: cfgQ.data?.agent?.custom_acp?.command?.trim() ?? '',
                  })}
                </p>
              )}
              {install && notInstalled(shown) && !unconfigured(shown) && shown !== CUSTOM && (
                  ...
                  capabilityLines(shown).every(line => !line.available) ? (
                    ...
                    <>
                      {i18nT('pages.developer.agentBackendTab.card_supports_n_of_m', {
                        ...
                        total: capabilityLines(shown).filter(line => line.measured !== false)
                          .length,
                      })}
                      {capabilityLines(shown).filter(line => line.measured === false).length >
                        0 && (
  ...
  return (
    ...
    >
      ...
      {/* What the operator is asserting by saving, stated where they save it: the
          ...
          the option is advertised and applied but not what it does. The first
          sentence -- what is and is not checked -- carries the paragraph's weight,
          so it is set in the strong text colour rather than filed with the fine
          print; the reader who skims stops on the claim the rest depends on. */}
      <p
        className="mb-0 text-[11px] leading-relaxed text-muted"
        data-testid="custom-acp-attestation"
      >
        <span className="text-text-strong">
          {i18nT('pages.developer.agentBackendTab.custom_attestation_lead')}
        </span>{' '}
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        {savedMessage && (
          <span role="status" className="inline-flex items-center gap-1 text-[11px] text-muted">
            {/* The glyph marks this as a message: two short texts side by side at the
                end of a button row read as two buttons to a blind reader, and a
                check in front is what tells "Saved." apart from "Save". Decorative
                for assistive tech -- role="status" already says what it is. */}
