...
export default function TeamDialog({
  ...
  const membersChanged =
    picked.size !== initialPicked.length || initialPicked.some((n) => !picked.has(n))
  ...
  const save = useMutation({
    mutationFn: async (): Promise<CrewTeam> => {
      ...
      // Edit sends ONLY what this dialog changed. The route leaves an omitted
      // field alone, so a rename from a dialog opened before another tab moved
      // a crewmate does not write that tab's membership back over the newer
      // one (and the reverse: a re-membering never carries a stale name).
      const body: { name?: string; members?: string[] } = {}
      if (nameChanged) body.name = trimmed
      if (membersChanged) body.members = orderedPicks()
