...
export default function MembersPage() {
  ...
  const activeSlotLastTs = useAppSelector(
    (s) => (chatSlot ? s.dashboard.slots.find(sl => sl.key === chatSlot)?.last_ts : undefined),
  )
  ...
  useEffect(() => {
    ...
    if (chatSlot && pageVisible) {
      if (activeSlotUnread) dispatch(markSlotRead(chatSlot))
      emitSlotRead(chatSlot, activeSlotLastTs)
    }
  }, [chatSlot, activeSlotUnread, pageVisible, activeSlotLastTs, dispatch])
  ...
  useEffect(() => {
    if (!chatSlot || !pageVisible) return
    setViewedThreadSlot(chatSlot)
    return () => {
      ...
      flushSlotRead(chatSlot)
      clearViewedThreadSlot(chatSlot)
    }
  }, [chatSlot, pageVisible])
