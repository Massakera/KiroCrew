 ...
 *   ?absent=NAME       a command the machine does not have: a record naming it is
 *                      registered but its probe answers "missing", the way the
 *                      gateway's does for a command the PATH search cannot find
 *   ?theme=dark|light
 *
...
const absent = (params.get('absent') || '').trim()
...
// A registered record whose command the machine lacks: the gateway probes the
// launch row and names the command as the missing component, with the record's own
// remedy (there is no installer Crew can suggest for a harness it did not choose).
const commandAbsent = () => configured() && absent !== '' && stored!.command.trim() === absent
const customRow = () =>
  commandAbsent()
    ? {
        ...CUSTOM_ROWS.configured,
        installed: 'missing',
        missing_components: [absent],
        install_command: `install whatever provides '${absent}', or correct agent.custom_acp.command in config.json`,
      }
    : CUSTOM_ROWS[configured() ? 'configured' : 'empty']
