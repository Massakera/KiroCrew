...
async def api_members(request: web.Request) -> web.Response:
    ...
    # Perpetual mode, per row, from the live loop registry: an O(1) in-memory
    # read per slot, so it fits this endpoint's "costs no IO" budget. The
    # roster and the team view read it as the crewmate's restart policy --
    # "on" while a loop on its own thread is active, "off" while a loop
    # record exists but is paused (the reason lives on the detail page),
    ...
    for row in rows:
        ...
        row["perpetual"] = perpetual_state_of(nudge_svc, slot_key)
...
def perpetual_state_of(svc: "AutoNudgeService | None", slot_key: str) -> str:
    """The roster's reading of one crewmate's Perpetual mode: on / off / none.

    ``on`` = a loop on the crewmate's own thread is active (whoever armed it:
    the switch reads ON for an active finite loop too -- "on" is "waking on
    its own", not "uncapped"; the detail page shows that loop's wake count
    against its cap, and the owner's ON lifts the cap only once the loop has
    stopped, since a live loop the member is driving is never rewritten under
    it). ``off`` = a loop record
    exists and is paused -- the owner's OFF, a cap, a budget or the crewmate's
    own stop; the reason itself is the detail page's to show. ``none`` = no
    record, or a structured monitor (which the switch never converts), or no
    bound thread, or no service.
    """
    ...
    if loop is None or is_structured_monitor_loop(loop):
        return "none"
    return "on" if loop.active else "off"
...
#: One lock per member slot for the switch's mutations: it serializes
#: concurrent presses of the switch on one member (and the identity re-check +
#: loop re-read + takeover steps of each) against each other, AND against the
#: member's own ``autonudge_stop`` / ``monitor_stop`` on a member slot
#: (``session_directive_apply._stop_resolved_loop`` takes the same lock): a
#: stop landing between the takeover's entry write and its resume would revoke
#: the entry under a loop then reported ON, every wake refused. ``monitor_update``
#: and the fire path go through the nudge service's own locks, not this one;
#: the takeover therefore still never assumes the loop it read is the loop that
#: is still there, and its rollback targets exactly the loop id and the takeover
