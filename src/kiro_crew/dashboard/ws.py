...
async def _handle_eventlog_frame(
    ...
    # Offloaded when COLD. `may_use_kind` is sync because one of its callers is a
    # sync frame filter, and on a cold cache it reads this app's metadata and
    # manifest -- file IO, on the serving loop, which is what `resolve_unit` below
    # is already offloaded to avoid. `is_cached` is a dict read, so the warm path
    # (every subscribe that is not the first after a lifecycle event) still answers
    # inline with no executor hop.
    if grants.is_cached(ws_app):
        kind_granted = grants.may_use_kind(ws_app, kind)
    else:
