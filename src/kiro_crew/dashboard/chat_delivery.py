...
def queue_entry_view(item: dict[str, Any]) -> dict[str, Any]:
    ...
    One serializer for the three slot-detail ``queue[]`` sites, so the lists
    cannot be echoed on one and dropped on another. Without them the client
    rebuilds the entry from ``content`` alone and a cancel falls back to a
    whitespace-bounded marker parse, which truncates a spaced path or leaves
    the marker in the composer verbatim. The ``meta`` key is the same one the
    ``queue_push`` and ``queue_pop`` frames use for these lists, so the client
    reads one shape however the entry reaches it; it is omitted, not emptied,
    for an entry without attachments so that entry's shape is unchanged. The
    lists pass :func:`attachment_meta`, which redacts each path like the
    content beside it.
    """
