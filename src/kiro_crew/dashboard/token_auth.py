...
async def _enforce_app_scope(request: web.Request, app_name: str, path: str) -> web.Response | None:
    ...
    # Warming is BOUNDED, so it can return with the cache still cold when a grant
    # generation lands during each of its hops. The decision below reads
    # ``permissions.api`` and the contributions declaration, so on a cold cache it
    # is a synchronous manifest read -- and running that here would put file IO on
    # the serving loop, which ``no-blocking-call-on-event-loop`` forbids outright.
    #
    # So the cold case takes an executor hop and the warm case does not.
    # ``_app_scope_is_cold`` is a dict read plus a counter read, which is why this
    # costs nothing on the overwhelming majority of requests. Failing closed
    # instead was the other option and is worse: it would 403 a legitimate app for
    # the duration of unrelated lifecycle churn.
    if _app_scope_is_cold(app_name):
        allowed = await asyncio.to_thread(app_token_path_allowed, app_name, path)
    else:
        allowed = app_token_path_allowed(app_name, path)
    if allowed:
        return None
