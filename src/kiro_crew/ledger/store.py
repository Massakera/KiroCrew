...
@dataclass(frozen=True)
class _OpenTail:
    """What a session ledger was left holding open, in first-seen order.
...
def _close_interrupted_tail(
    ...
    *,
    ...
    when the ledger is LOADED after an interruption answers that once, in the
    ...
    *child_gone* predicate reports finished. A child is not turn-scoped, so a
    ...
    with _open_lock(_lock_path(kind, unit_id)):
        ...
        for entry in _closer_entries(opened, tail.last_seq + 1):
            ...
            written += 1
    logger.info(
        "closed an interrupted tail in session ledger %r: %d closer(s) appended",
        unit_id,
        written,
    )
    return written
