...
class KasHarness(MembershipHarness):
    ...
    def activation_refusal(self, agent: str, resp: dict[str, Any]) -> str | None:
        """Refuse an id the engine advertises as ITS OWN agent rather than ours.

        The definition Crew sent rode ``_meta.kiro.customAgents``, so the
        advertised mode of that id must carry ``origin: client``. A positive
        other stamp (``bundled``) means the engine kept a built-in agent under
        the id and discarded the client entry -- the one shape the
        ...
        engine adds fails loudly instead of silently substituting. An absent
        stamp is not evidence either way and never refuses.
        """
        ...
        # replacement stamped ``bundled``) shows up here as the same wrong value on
        # every crewmate, instead of as a rename remedy that cannot help.
        logger.warning(
            "agent %r: the engine advertises this id as its own definition "
            "(origin=%r, not 'client'); refusing to activate it",
            agent,
            origin,
        )
        return (
            f"Rename this crewmate's template: {agent!r} is reserved for a built-in "
