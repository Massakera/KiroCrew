...
def _seal_pi_bridge_extension() -> str:
    ...
    the caller treats that as "no bridge this session", not as a refusal. The
    ``_bridge`` infix keeps the artifact under the ``kirocrew_pi_gate_<pid>``
    prefix the gate-dir sweep (``sandbox._PI_GATE_DIR_ARTIFACTS``) claims.
    """
    return _seal_gate_extension(
        pi_bridge_extension_path(),
        PI_BRIDGE_EXTENSION_SHA256,
        artifact_dir=_pi_gate_artifact_dir(),
        sealed_name=f"kirocrew_pi_gate_{os.getpid()}_bridge.ts",
        stage_prefix=f"kirocrew_pi_gate_{os.getpid()}_bridge_",
