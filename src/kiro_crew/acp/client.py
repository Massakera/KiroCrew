...
# Absolute ceiling on one response write (lock wait + drain together), on top
# of the no-progress window. The window alone accepts any reader that clears
# the progress floor, so a reader trickling just above it could hold a deny
# path for hours; the ceiling ends that. Derived, not guessed: the largest
# frame this transport sends is a prompt carrying several images (~30 MiB,
# _STDOUT_BUFFER_LIMIT-scale); a reader that has not finished it in fifteen
# minutes is sustaining under ~40 KiB/s, which no healthy backend does on a
# local pipe. Fifteen minutes is also far below every turn deadline (hours), so
# the caller still gets the fail-fast recovery this bound exists for.
_RESPONSE_WRITE_CEILING_SECS = 900.0
...
# not a stall there, and the only signal left is elapsed time. With nothing to
# extend the wait on, the only defensible window is the ceiling itself: the
# same largest-frame-at-slowest-healthy-rate derivation, so a healthy Windows
# reader mid-frame is never killed early. Platform-limited, like the Windows
# watchdog: a dead reader is detected in 15 minutes there instead of 5 seconds.
_RESPONSE_WRITE_UNOBSERVABLE_BOUND_SECS = _RESPONSE_WRITE_CEILING_SECS
...
async def await_under_no_progress_bound(
    ...
    *,
    ...
    buffer showed no activity for ``bound_secs``, or when ``deadline`` (a
    loop-time instant, the caller's absolute ceiling) passed. Activity at a window's end
    ...
    try:
        while True:
            ...
            if deadline is not None:
                timeout = min(window, max(0.0, deadline - loop.time()))
            try:
...
async def write_response_frame_bounded(
    ...
    *,
    ...
    """
    deadline = asyncio.get_running_loop().time() + _RESPONSE_WRITE_CEILING_SECS
    acquire: asyncio.Future[bool] = asyncio.ensure_future(lock.acquire())
    try:
        acquired = await await_under_no_progress_bound(
            acquire, stdin, bound_secs=bound_secs, deadline=deadline
        )
    ...
    try:
        if before_write is not None:
            before_write()
        stdin.write(data)
        return await await_under_no_progress_bound(
            stdin.drain(), stdin, bound_secs=bound_secs, deadline=deadline
        )
    finally:
        lock.release()
    ...
    *,
    ...
    try:
        if before_write is not None:
            before_write()
        stdin.write(data)
        drained = await await_under_no_progress_bound(
            stdin.drain(), stdin, bound_secs=bound_secs, deadline=deadline
        )
...
class AcpClient:
    ...
    async def _write_response_bounded(self, data: bytes, request_id: str | int) -> None:
        ...
        logger.warning(
            "ACP stdin stalled: no write progress for %gs (ceiling %gs) while delivering "
            "response to req=%s; treating the backend as dead",
            window,
            _RESPONSE_WRITE_CEILING_SECS,
            safe_id,
        )
