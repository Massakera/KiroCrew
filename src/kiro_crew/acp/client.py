...
class AcpClient:
    ...
    def _reset_state(self) -> None:
        ...
        # The custom-gate tripwire's memory belongs to the process that just died.
        # Its ids were minted by that harness; a replacement harness minting the
        # same id must be asked again, not inherit the old approval -- an
        # ``asked`` entry that survived here would let ``_tripwire_custom_gate``
        # read a call it never saw as gated. The snapshot goes with it: the spawn
        # arm retakes it from the live spec, and a custom session that reaches
        # ``_initialize_session`` without one is refused rather than armed from
        # a dead process's copy.
        self._custom_gate_asked_ids = set()
        self._custom_gate_request_tool = {}
        self._custom_gate_denied_ids = set()
        self._custom_gate_passive_ids = set()
        self._custom_gate_overlong_id_len = 0
        self._custom_gate_snapshot = None
