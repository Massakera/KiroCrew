...
async def authorize_and_add_nudge(
    *,
    ...
    try:
        ...
        # shutdown) must not leave the trust entry it just wrote standing over
        # a loop that may never have been committed: an orphaned entry would
        # vouch for a forged loop of that id on that slot. The service's own
        # add is shielded, so a loop that did commit sits without its entry and
        # is refused at fire time -- the safe side; the owner's next press
        # re-records it. Joined, so the cancel does not outrun the write.
