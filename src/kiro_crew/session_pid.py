...
def _cmdline_is_the_custom_harness(cmdline: bytes) -> bool:
    ...
    RESIDUAL -- reconfiguration. The identity is the CURRENT spec's, so a harness
    spawned under one command that outlives a gateway crash, after which config
    names a different command or none, is not recognised by the sweep and lingers
    until the operator kills it. That process is the one the operator already
    launched, running with what it already had; the gap spares it, and can never
    kill an unrelated one. It is stated rather than closed because the closing move
    -- recording each PID's launch identity and honouring the record at reclaim --
    is the thing :func:`_kill_pid_tree` forbids: the tracking file is same-uid
    writable, so a recorded identity may not authorize a signal. ``session.md``
    carries the same paragraph beside the Windows residual.
    """
