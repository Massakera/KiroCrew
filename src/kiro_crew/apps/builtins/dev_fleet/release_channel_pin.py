...
async def worktree_state(path: str, resolved: dict) -> dict:
    ...
    if out["detached"] is True:
        # Independent of whether the TIP resolved. The tree's own release is a
        # fact about the tree, answered from the local mirror with no network
        # call, so a failed tip lookup must not turn a checkout that sits on a
        # published release into "not on a release" -- that would misreport the
        # one thing the badge exists to say, at exactly the moment the operator
        # also has an error notice to read. With no tip, `at_tip` stays False
        # and the caller says the tip is unknown.
