...
def _pinned_ancestors(path: Path) -> Path:
    ...
    name is left alone, or the walk follows a link planted AT the very
    directory it is pinning. ``realpath``, not ``Path.resolve``, which raises
    RuntimeError on a cycle and escapes the OSError callers refuse with.
    """
