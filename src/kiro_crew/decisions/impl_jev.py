...
Wire format taken from ``https://docs.typesafe.ai/api`` (read 2026-09-17), not
inferred:
...
* ``Choice.options`` -> ``criteria`` is a MAP of option to rubric text, and the
  ...
  API requires a map.
* ``Score(lo, hi)`` -> ``criteria`` is an ORDERED ARRAY of level descriptions,
  indexed from 0. So the ladder is sent as ``hi - lo + 1`` entries and the
  returned ``score`` (0-based, possibly fractional) is shifted back by ``lo`` on
  the way out. Without that shift a ``Score(lo=1, hi=5)`` would answer 0..4 and
  every consumer would be off by one in the direction that still looks
  plausible.
...
def resolve_api_key(raw: str) -> str:
    ...
    vault's own exception text can name a path.
    """
    value = (raw or "").strip()
    if not value:
        return ""
    ...
    try:
        ...
        logger.warning("decisions: vault read for %s failed: %s", name, exc)
        return ""
    ...
    for q in questions:
        if isinstance(q, Choice):
            ...
            wire_questions[q.id] = {
                "type": "score",
                "instructions": q.prompt,
                # Ordered level descriptions, index 0 upward. The rung's own
                # number in the caller's units is the most honest description
                # available without asking the point file for prose.
                "criteria": [str(level) for level in range(q.lo, q.hi + 1)],
            }
...
def _answer_from_wire(q: Question, raw: dict) -> Answer:
    ...
    if isinstance(q, Score):
        ...
        if not isinstance(score, (int, float)) or isinstance(score, bool):
            raise JevProtocolError(f"answer for {q.id!r} has no numeric 'score'")
        # Shift 0-based levels back onto the caller's ladder, and take ``p`` from
        # the heaviest rung -- a fractional score sits between rungs, so there is
        # no single rung whose probability IS the answer's.
        top = max(probabilities.values(), default=0.0) if probabilities else 0.0
        return Answer(
            id=q.id,
            value=q.lo + float(score),
            p=_as_float(top),
            confidence=confidence,
        )
...
class JevOracle:
    ...
    async def ask(self, state: dict | str, questions: list[Question]) -> OracleResult:
        ...
        api_key = resolve_api_key(self._api_key_setting)
        if not api_key:
