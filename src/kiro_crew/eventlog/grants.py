...
* **A unit KIND is authority, so it is not taken from the manifest alone.** The
  ...
  with ``InstalledApp.approvedUnits``, which only an install or an update writes,
  so a kind must be both still declared AND already approved.
"""
    ...
    try:
        ...
        if is_app_enabled(app):
            ...
            if manifest is not None:
                ...
                # docstring. `approved_unit_kinds` is empty for an app whose record
                # predates the field, so a legacy install contributes to no kind
                # until an operator re-approves -- the app keeps every other grant,
                # and `units_pending_approval` is what shows them the difference.
