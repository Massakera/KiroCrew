...
class DecisionProviderConfig:
    ...
    kind: str = field(
        ...
        metadata=_meta(
            "Kind",
            "Which provider protocol to speak. Only 'typesafe' (TypeSafe System "
            "One / Jev) is implemented; the field exists so a second provider is "
            "a value here rather than a new section.",
            enum=["typesafe"],
        ),
    )
    endpoint: str = field(
...
class DecisionPointConfig:
    ...
    impl: str = field(
        default=DECISION_IMPL_JEV,
        metadata=_meta(
            "Implementation",
            "Which implementation answers this point: 'jev' (the System One "
            "provider) or 'llm' (the same questions asked of a background chat "
            "turn, for comparison). Anything unrecognised is read as 'jev'.",
...
class DecisionsConfig:
    ...
    ``preview`` is the master switch and lives HERE, in config.json, rather than
    ...
    preview: bool = field(
        ...
        metadata=_meta(
            "Preview",
            "Master switch for the DecisionOracle seam. False (the default) "
    ...
    def from_raw(cls, section: object) -> "DecisionsConfig":
        ...
        provider = DecisionProviderConfig(
            # ``kind`` is read back verbatim: the gate resolves the
            # implementation from the POINT's ``impl``, so an unknown kind cannot
            # misroute a request, and normalizing it here would hide an
            # operator's typo behind a working default.
            kind=str(raw_provider.get("kind", DecisionProviderConfig.kind) or ""),
        ...
        for name, entry in raw_points.items():
            ...
            impl = str(entry.get("impl", DECISION_IMPL_JEV) or "").strip().lower()
            if impl not in DECISION_IMPLS:
                impl = DECISION_IMPL_JEV
        ...
        return cls(
            ...
            # master switch for sending conversation state off the machine, so an
