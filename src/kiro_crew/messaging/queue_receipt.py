...
class ReceiptQueue:
    ...
    async def flip_answering_locked(
        ...
        the part of ``answered`` that this bubble was actually DISPLAYING -- a cap can
        leave the rest of its messages in the queue, and a record naming those would
        claim them answered.

        ``answered`` carries the messages of ONE conversation: the one ``surface``
        names. Every drain collapses a turn from entries sharing a sender and a
        place, taken from the first entry it reads, and defers the rest -- Teams,
        Telegram and Discord on ``_QueuedOrigin.sender_key``, Webex on
        ``_QueuedPlace.sender_key`` -- and each builds this ``surface`` from that same
        entry. So a co-tenant's copy is never in ``answered``, and this conversation's
        copies are not ambiguous with anyone: they are the only copies there are. The
        tally below therefore constrains the CO-TENANTS, who take what this
        conversation's split did not, and never this conversation itself.
        ...
        cap. Only the first may be finalized, and the two are told apart
        by evidence already in hand -- the queue is provably empty when the drain
        re-enqueued nothing of this conversation's and no text here is one a
        co-tenant could account for. Otherwise this bubble stays
        live, exactly as a co-tenant in the same state does. A stale "⏳ Queued" is
        corrected by this
        conversation's own next turn; "▶️ Now answering" printed over a message
        still waiting is a lie the record keeps.

        ``deferred`` is what the drain re-enqueued past a cap, counted for THIS
        conversation by every caller. It is shown as-is
        while this conversation is ALONE on the session key. Once a co-tenant holds a
        receipt the body states only what the split proved this
        conversation did not get answered, so a miscounted total cannot reach another
        person's chat as a promise of messages they never sent.

        The session's OTHER receipts are then reconciled against ``answered``,
        because a drain that dequeues by session key alone
        ...
        declines to use it for a CO-TENANT: a text the session wants more copies of
        than were answered, and that more than one conversation is displaying, is
        claimed by no co-tenant, leaving those bubbles live rather than
        finalizing whichever the dict happened to yield first, and never
        presenting as answered a copy that may have been someone else's. Leaving
        one live is self-correcting -- that conversation's own next turn resolves
        it -- where finalizing the wrong chat is a lie that never is. This
        conversation's own copies are outside that test, because ``answered`` holds
        no copy but its own: subjecting them to it made the self-correction
        circular, since the next turn it defers to is the very turn that would have
        to claim them.
        ...
        receipt = self._receipts.get(entry)
        if receipt is None:
            # No entry at all says nothing about what this conversation has
            # QUEUED: on a drain that runs under the turn opener's envelope its
            # own messages are invisible here, so a co-tenant cannot be finalised
            ...
            return
        if receipt.final_body is not None:
            # A terminal entry is a retry handle, not a queue entry: its messages
            # left the queue in an earlier transition, and a later message would
            # have replaced it with a live bubble, so this conversation is KNOWN
            # to hold nothing. It therefore contributes nothing to the tally, and
            # unlike the absent case above there is nothing unseen to net out --
            # so retry the record it owes, release the key, and still reconcile
            # the co-tenants whose messages this turn did answer. Returning here
            # instead would leave a consumed co-tenant reading "⏳ Queued".
            await self._retry_terminal(entry, receipt)
            receipt = None
        else:
        ...
        # each conversation takes its own. Short of that the copies cannot be
        # attributed by text, so no co-tenant claims one -- claiming it would spend
        # what another conversation's own message accounts for. Holders are counted
        # per conversation, so a conversation that repeated itself is never
        ...
        if receipt is not None:
            # UNFILTERED, and that is the whole point of the scope contract above:
            # every copy in ``answered`` is this conversation's, so there is nothing
            # for it to be ambiguous WITH. Filtering here instead made two ordinary
            # people who both typed "ok" permanently unresolvable -- each one's own
            # drain answered its own message, found that text shared, declined to
            # claim it, and left the bubble on "⏳ Queued"; the next turn read the
            ...
            # this conversation's and no text here is one a co-tenant could account
            # for, leaving a rendering mismatch as the sole explanation. Short of
            # that a message of this conversation's may still be waiting, so the
            # bubble is left exactly as it is, the same outcome a co-tenant in this
            # state gets below. Stale reads "⏳ Queued" until this conversation's own
