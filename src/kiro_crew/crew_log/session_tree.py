...
def edge_supersedes(
    ...
    held: EdgeRecord,
    log_rank: Mapping[str, tuple[int, str]] | None = None,
) -> bool:
    ...
    * **Different logs.** Seqs are not comparable: a slot outlives its ACP session and
      ...
      *log_rank* is how a caller states which that is -- the same ``(created_at, sid)``
      ordering :func:`fold_tree` already sorts records by, so the decisions inherit the
      log order the tree is built from rather than inventing a second one. A log the
      caller cannot rank falls back to ``(at, sid)``: it is the weakest answer here and
      it is confined to the case where nothing better exists.
...
def latest_edges(
    edges: Iterable[EdgeRecord], log_rank: Mapping[str, tuple[int, str]] | None = None
) -> dict[str, EdgeRecord]:
...
def log_rank_of(records: Iterable[OpenedRecord]) -> dict[str, tuple[int, str]]:
    """Each log's place in the order :func:`fold_tree` already folds them in.

    ``sid -> (created_at, sid)``, which is the key that function sorts records by. It
    exists so the decisions and the creating citations are placed by ONE log order
    rather than two: a second ordering rule here could put a slot's decision in a log
    the record fold considers older, and the tree would then disagree with itself.
    """
    return {record.sid: (record.created_at, record.sid) for record in records if record.sid}
...
def fold_tree(
    ...
    *edges* are the later decisions -- an adoption, a release -- and each one
    ...
    for slot, edge in latest_edges(edges, log_rank_of(ordered)).items():
        if slot in has_log:
