...
class TestGateTripwire:
    ...
    def test_an_approval_does_not_survive_the_process_it_was_given_to(
        self, clean_registry, tmp_path, monkeypatch
    ):
        """Approved id, child dies before its terminal update, replacement harness
        mints the same id: the replacement must be asked again. The tripwire's
        sets, the request map, the over-long marker and the spec snapshot all
        belong to the dead process and are dropped with it."""
        client, killed = self._armed(tmp_path, monkeypatch)
        client._custom_gate_snapshot = ("mode", "read-only")
        client._extract_tool_event(_tool_call("c9", kind="execute"))
        client._build_permission_event(_permission("c9"))
        asyncio.run(client.approve_tool(7))
        client._extract_tool_event(_tool_call("p1", kind="read"))
        client._custom_gate_denied_ids.add("d1")
        client._custom_gate_overlong_id_len = 300
        assert "c9" in client._custom_gate_asked_ids

        client._reset_state()

        assert client._custom_gate_asked_ids == set()
        assert client._custom_gate_request_tool == {}
        assert client._custom_gate_denied_ids == set()
        assert client._custom_gate_passive_ids == set()
        assert client._custom_gate_overlong_id_len == 0
        assert client._custom_gate_snapshot is None

        # The replacement reuses the id without asking: the stale approval is gone,
        # so this is an unasked completed call and the harness is stopped.
        client._extract_tool_event(_tool_call("c9", kind="execute"))
        with pytest.raises(AcpToolGateUnroutable):
            asyncio.run(client._tripwire_custom_gate(_update("c9")))
        assert killed == [True]
