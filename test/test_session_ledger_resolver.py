...
def test_both_background_helpers_pin_the_owner_before_their_first_await():
    ...
    for node in ast.walk(tree):
        ...
        pins = [
            ...
            and any(isinstance(t, ast.Name) and t.id == "_ledger_owner" for t in n.targets)
        ]
        if not pins:
            continue
        ...
        assert min(pins) < min(awaits), (
            f"{node.name} resolves _ledger_owner at line {min(pins)}, after its first "
...
def test_a_replayed_queued_child_is_repinned_with_an_unobserved_turn(monkeypatch):
    ...
    _GateMixin._record_ledger_dispatch(stub, info, from_queue=True)
