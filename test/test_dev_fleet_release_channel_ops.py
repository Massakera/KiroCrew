...
async def test_discard_drops_only_the_staging_record_and_spares_an_unmounted_worktree(
    ...
    def registered_names(porcelain: str) -> set[str]:
        # Basenames, not full paths: git prints forward-slash paths on every
        # platform while ``str(tmp_path / ...)`` is backslashed on Windows, so a
        # substring match on the full path fails there for a record that exists.
        return {
            line.split(" ", 1)[1].rstrip("/").rsplit("/", 1)[-1]
    ...
    assert rc == 0
    names = registered_names(listed)
    assert {staging.name, unrelated.name} <= names, f"fixture: both records must exist, got {names}"
    ...
    assert rc == 0
    names = registered_names(listed)
    assert staging.name not in names, "the staging record is this call's to drop"
    assert unrelated.name in names, "an unrelated worktree's record must survive the cleanup"
...
@pytest.mark.asyncio
async def test_an_adopted_row_keeps_its_own_release_when_the_tip_lookup_fails(repo, monkeypatch):
    """The badge must not say "not on a release" because the TIP could not be read.

    The tree's release comes from the local mirror and is a fact about the tree;
    the tip is a fact about the channel. When only the second fails, the row
    carries the resolver error, keeps the release it holds, and leaves ``at_tip``
    unknown rather than asserting either answer. This is the state frame 07 of
    the capture harness shows, so the backend has to be able to produce it.
    """
    resolver_error = "git ls-remote --tags: fatal: unable to access remote"

    async def failed_resolve(*, repo):
        return {"ok": False, "error": resolver_error}

    async def own_release(path, resolved):
        assert resolved == {}, "a failed resolve hands the probe nothing to compare against"
        return {"at_tip": False, "detached": True, "version": "0.5.0"}

    monkeypatch.setattr(release_channel_pin, "resolve", failed_resolve)
    monkeypatch.setattr(release_channel_pin, "worktree_state", own_release)
    row = await fleet_state._release_channel(
        [{"path": "/wt/release-channel-stable", "is_main": False}]
    )
    ...
    assert row["worktree"] == release_channel_pin.WORKTREE_NAME
    assert row["version"] == "0.5.0"
    assert row["tip_version"] is None
    assert row["at_tip"] is None
    assert row["error"] == resolver_error
