...
@pytest.mark.asyncio
async def test_skills_select_swallows_a_failing_candidate_provider(fake_core, spy, monkeypatch):
...
@pytest.mark.asyncio
async def test_skills_select_payload_shape(fake_core, spy, monkeypatch):
    monkeypatch.setattr(skills_select, "_decide", spy)
    candidates = [
    ...
    assert [q.id for q in questions] == ["pick", "any_relevant"]
    assert questions[0].options == ["brazil", "crux", "none"]
    assert kwargs["baseline"] == {"pick": ["crux"]}
    assert kwargs["session_key"] == "chat-1"


@pytest.mark.asyncio
async def test_skills_select_accepts_a_lazy_candidate_provider(fake_core, spy, monkeypatch):
    """The sync call site passes a callable so enumeration is off its critical path."""
    monkeypatch.setattr(skills_select, "_decide", spy)
    calls = []
    ...
    assert questions[0].options == ["tst", "none"]


@pytest.mark.asyncio
async def test_skills_select_caps_the_candidate_menu(fake_core, spy, monkeypatch):
    monkeypatch.setattr(skills_select, "_decide", spy)
