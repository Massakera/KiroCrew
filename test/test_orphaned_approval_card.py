...
class TestRunnerBackstopContract:
    ...
    def test_ledger_decision_is_folded_to_the_schema_enum(self) -> None:
        ...
        src = inspect.getsource(chat_runner._run_chat)
        assert (
            "decision=_ledger_decision" in src
        ...
        assigned = set(re.findall(r'_ledger_decision = "([a-z_]+)"', src))
    ...
    def test_a_stopped_approval_is_not_recorded_as_a_person(self) -> None:
        ...
        emit = src.index("session_ledger_emit.on_approval_decided(")
        assert (
            read < discard < emit
