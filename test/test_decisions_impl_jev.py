...
(read 2026-09-17). If that page changes, these tests are the record of what it
said, and ``_to_wire``/``_from_wire`` are the only two functions to edit.
"""
...
SCORE = Score(id="frustration", prompt="How frustrated?", lo=1, hi=3)
...
class TestRequestShape:
    ...
    def test_every_question_goes_in_one_request(self):
        rec = _Recorder(
            body=_ok_body(
                {
                    "department": {
        ...
        asyncio.run(_run(rec, [CHOICE, SCORE, NOUL]))
        assert len(rec.requests) == 1, "one round trip, not three"
        assert set(rec.requests[0]["questions"]) == {"department", "frustration", "is_urgent"}
    ...
    def test_score_criteria_is_an_ordered_array_covering_the_ladder(self):
        rec = _Recorder(
            body=_ok_body(
                {
                    "frustration": {
        ...
        asyncio.run(_run(rec, [Score(id="frustration", prompt="rate", lo=1, hi=5)]))
        q = rec.requests[0]["questions"]["frustration"]
        assert q["type"] == "score"
        assert q["criteria"] == ["1", "2", "3", "4", "5"], "five inclusive rungs, in order"
...
class TestSuccessfulParse:
    ...
    def test_score_answer_is_shifted_onto_the_questions_own_ladder(self):
        """The API answers 0-based; a ``Score(lo=1, hi=3)`` must come back 1-based.

        Without the shift every consumer is off by one in the direction that
        still looks plausible, which is why this is asserted rather than assumed.
        """
        rec = _Recorder(
            body=_ok_body(
                {
        ...
        answer = asyncio.run(_run(rec, [SCORE])).answers["frustration"]
        assert answer.value == pytest.approx(2.6), "lo(1) + score(1.6)"
        assert answer.p == pytest.approx(0.65), "the heaviest rung"
        assert answer.confidence == pytest.approx(0.78)
...
class TestFailures:
    ...
    def test_a_score_answer_without_a_number_raises(self):
        rec = _Recorder(body=_ok_body({"frustration": {"type": "score", "score": "high"}}))
        with pytest.raises(JevProtocolError, match="no numeric 'score'"):
            asyncio.run(_run(rec, [SCORE]))
...
class TestProviderDefaults:
    ...
    def test_request_body_is_json_serialisable_as_sent(self):
        ...
        json.dumps(_to_wire("hi", "jev-latest", [CHOICE, SCORE, NOUL]))
