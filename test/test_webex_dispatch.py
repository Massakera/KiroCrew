...
class TestQueueReceiptThreadIdentity:
    ...
    async def test_the_drain_flips_the_thread_it_dequeued_not_the_opener(
        ...
        with caplog.at_level(logging.WARNING, logger="kiro_crew.messaging.queue_receipt"):
            await d._drain_queue(key, self._in_thread("THREAD1", "opener"))
    ...
    async def test_an_answered_uncaptioned_attachment_is_not_left_queued(self, monkeypatch) -> None:
        ...
        # What the drain REPORTS, observed directly. The outcome alone is not
        # enough: with nothing of this sender's deferred, the registry's
        # rendering-mismatch fallback finalizes the bubble from its own texts, so a
        # drain reporting the raw empty text would still flip it -- and would still
        # be wrong for a sender whose next message re-grows an answered bubble.
        reported: list[list[str]] = []
        real_flip = d._queue.flip_answering_locked

        async def _spy(session_key, surface, answered, deferred=0):
            reported.append(list(answered))
            await real_flip(session_key, surface, answered, deferred)

        monkeypatch.setattr(d._queue, "flip_answering_locked", _spy)

        sessions._busy = False
        await d._drain_queue(key, self._in_thread("THREAD1", "opener"))

        assert any(webex_dispatch._QUEUED_ATTACHMENT_LABEL in a for a in reported), (
            "the drain reported the attachment's raw empty text, which matches "
            "nothing in the bubble the receipt registered under the placeholder"
        )
        flipped = [mid for mid, _room, body in client.edits if "Now answering" in body]
        assert receipt_id in flipped, (
    ...
    async def test_a_thread_toggle_between_enqueue_and_drain_still_finds_the_receipt(
        ...
        sessions._busy = False
        with caplog.at_level(logging.WARNING, logger="kiro_crew.messaging.queue_receipt"):
            await d._drain_queue(key, self._in_thread("THREAD2", "in thread"))
