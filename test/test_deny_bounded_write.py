...
def short_bound(monkeypatch: pytest.MonkeyPatch) -> float:
    ...
    # The absolute ceiling stays far above the window here (as in production).
    monkeypatch.setattr(acp_client, "_RESPONSE_WRITE_CEILING_SECS", _TEST_BOUND * 40, raising=False)
    return _TEST_BOUND
...
class TestCeiling:
    """The no-progress window accepts any reader above the floor; the ceiling
    ends a wait that clears the floor forever."""

    @pytest.mark.asyncio
    async def test_a_reader_trickling_above_the_floor_is_ended_by_the_ceiling(
        self, short_bound: float, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(acp_client, "_RESPONSE_WRITE_CEILING_SECS", short_bound * 3)
        floor = acp_client._RESPONSE_WRITE_MIN_PROGRESS_BYTES
        levels = [100 * floor - k * floor for k in range(100)]  # always above the floor
        stdin = _stdin_with_buffer(levels, drain_after=None)
        stdin.write = MagicMock()
        ok = await asyncio.wait_for(
            write_response_frame_bounded(stdin, asyncio.Lock(), b"{}\n", bound_secs=short_bound),
            timeout=_OUTER_GUARD,
        )
        assert ok is False
        assert 3 <= stdin.polls["n"] <= 6, "the ceiling did not end the wait near three windows"

    def test_unobservable_window_is_the_ceiling(self) -> None:
        # On a transport whose level cannot be read, nothing can extend a
        # shorter window, so the only defensible single window is the ceiling
        # -- same largest-frame derivation, never an early kill mid-frame.
        assert (
            acp_client._RESPONSE_WRITE_UNOBSERVABLE_BOUND_SECS
            == acp_client._RESPONSE_WRITE_CEILING_SECS
        )

    def test_ceiling_sizing_is_derived_from_the_largest_frame(self) -> None:
        # ~30 MiB at the slowest rate a healthy local pipe reader sustains
        # (~40 KiB/s) fits inside the ceiling; the ceiling stays far below any
        # turn deadline (hours).
        largest_frame_bytes = 30 * 1024 * 1024
        slowest_healthy_rate = 40 * 1024
        assert acp_client._RESPONSE_WRITE_CEILING_SECS >= largest_frame_bytes / slowest_healthy_rate
        assert acp_client._RESPONSE_WRITE_CEILING_SECS <= 3600
    ...
    def test_every_backend_id_log_site_passes_the_sanitized_id(self) -> None:
        ...
        for rel, needles in self.SITES.items():
            ...
            for needle in needles:
                ...
                for i in hits:
                    # The sanitized value may be bound a few lines above the
                    # format string (``safe_id = _loggable_request_id(...)``).
                    window = "\n".join(lines[max(0, i - 10) : i + 10])
                    slots = len(re.findall(r"\b(?:req|id|method)=%[sr]", lines[i]))
                    assert (
                        window.count("_loggable_request_id(") >= slots
                    ), f"{rel}:{i + 1} logs a raw backend value ({slots} slot(s))"
