...
class TestAnOrphanedRowIsNotRendered:
    ...
    def test_the_guard_allows_a_granted_key_and_refuses_an_unowned_one(self, monkeypatch):
        ...
        # A unit kind also needs the install record's approval, not just the manifest.
        monkeypatch.setattr(
...
# ---------------------------------------------------------------------------
# Unit-kind authority: the manifest declares, the install record approves
# ---------------------------------------------------------------------------
...
class TestAUnitKindNeedsBothADeclarationAndAnApproval:
    ...
    approval; every one of them passed before ``approvedUnits`` existed.

    Deliberately end-to-end against the real lifecycle functions and a real
    ``installed.json``: the defect lives in the relationship between two files on
    ...
    def test_a_record_written_before_the_field_existed_approves_no_kind(self, tmp_path, units_home):
        ...
        self._install_enabled(tmp_path, units=["member"])
        # Strip the field, which is exactly how a record written before it existed
        # reads on disk.
        record = app_dir(self.APP) / INSTALLED_META_FILENAME
        data = json.loads(record.read_text(encoding="utf-8"))
        assert data.pop("approvedUnits", None) == ["member"], "fixture must start approved"
...
class TestGrantChecksInTheWebSocketPathNeverBlockTheLoop:
    ...
    the read cannot be made async -- it has to happen off the loop or after a warm.
    This walks EVERY grant question in the module rather than the one line a review
    cited, so the next one added is covered before a reviewer sees it.
    """
    ...
    def _grant_calls(self, node):
        """Every ``grants.<question>(...)`` Call inside *node*."""
        out = []
        for sub in ast.walk(node):
            ...
            if (
                ...
                and func.attr in self.QUESTIONS
                and isinstance(func.value, ast.Name)
                and func.value.id == "grants"
            ):
                out.append(sub)
        return out

    def test_every_grant_question_is_offloaded_or_guarded_by_the_cache_check(self):
        tree = self._module_tree()
        checked = 0
        for fn in ast.walk(tree):
            if not isinstance(fn, ast.AsyncFunctionDef):
                continue
            calls = self._grant_calls(fn)
            if not calls:
                continue
            # Collect the line spans that are allowed to hold an INLINE question:
            # the body of an `if grants.is_cached(...)`, where the answer is a dict
            # read. Anything outside one of those must be an argument to
            # `asyncio.to_thread`.
            warm_spans = []
            for node in ast.walk(fn):
                if not isinstance(node, ast.If):
                    continue
                if any(
                    isinstance(c.func, ast.Attribute) and c.func.attr == "is_cached"
                    for c in ast.walk(node.test)
                    if isinstance(c, ast.Call)
                ):
                    for stmt in node.body:
                        warm_spans.append((stmt.lineno, stmt.end_lineno or stmt.lineno))
            offloaded = set()
            for node in ast.walk(fn):
                if (
            ...
            for call in calls:
                checked += 1
                name = call.func.attr
                inside_warm = any(lo <= call.lineno <= hi for lo, hi in warm_spans)
                assert inside_warm or name in offloaded, (
                    f"{fn.name}: grants.{name} at line {call.lineno} runs on the loop "
                    "without an is_cached guard and without asyncio.to_thread"
                )
