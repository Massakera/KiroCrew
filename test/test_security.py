...
class TestExfilUrlPathAndRawIp:
    ...
    # characters and stops only at whitespace and a double quote. Wrapper
    # punctuation is trimmed only through a matching terminal pair.
    ...
    @pytest.mark.parametrize(
        "delimiter",
        [
            pytest.param("<", id="less-than"),
            pytest.param(">", id="greater-than"),
            pytest.param("`", id="backtick"),
        ],
    )
    def test_raw_html_href_url_content_does_not_truncate(self, delimiter: str) -> None:
        text = f'<a href="https://evil.example/a{delimiter}b?data={self._ENTROPY_BLOB}">'
        result, warnings = redact_exfiltration_urls(text)
        assert warnings
        assert self._ENTROPY_BLOB not in result
        assert result == '<a href="[REDACTED: suspicious URL to evil.example]">'

    def test_angle_autolink_wrapper_trims_from_maximal_match(self) -> None:
        from kiro_crew.security.exfil import _URL_RE, _url_scan_span

        url = f"https://evil.example/path?data={self._ENTROPY_BLOB}"
        text = f"<{url}>"
        match = _URL_RE.search(text)
        assert match is not None
        assert match.group(0) == f"{url}>"
        scanned_url, path_and_query = _url_scan_span(match)
        assert scanned_url == url
        assert path_and_query == f"/path?data={self._ENTROPY_BLOB}"
        result, warnings = redact_exfiltration_urls(text)
        assert warnings
        assert result == "<[REDACTED: suspicious URL to evil.example]>"

    def test_backtick_wrapper_trims_from_maximal_match(self) -> None:
        from kiro_crew.security.exfil import _URL_RE, _url_scan_span
    ...
    def test_double_quote_stays_a_hard_terminator(self) -> None:
        # The double quote bounds quoted HTML attributes and compact structured
        # fields. Crossing it would absorb adjacent values into URL heuristics.
        benign = f'https://example.com/a"b?data={self._ENTROPY_BLOB}'
        assert not scan_exfiltration_urls(benign)
        before = 'https://evil.example.com/AKIAIOSFODNN7EXAMPLE"tail'
        assert scan_exfiltration_urls(before), "content before a quote must still be scanned"
    ...
    def test_single_quoted_url_redaction_preserves_adjacent_sha(self) -> None:
        sha = "da39a3ee5e6b4b0d3255bfef95601890afd80709"
        text = "{'url':'https://evil.example.com/p?data=" f"{self._ENTROPY_BLOB}','sha':'{sha}'}}"
        result, warnings = redact_exfiltration_urls(text)
        assert warnings
        assert result == (
            "{'url':'[REDACTED: suspicious URL to evil.example.com]'" f",'sha':'{sha}'}}"
        )

    @pytest.mark.parametrize(
        ("prefix", "suffix"),
        [
            pytest.param("('", "')", id="quote-paren"),
            pytest.param("('", "'),", id="quote-paren-comma"),
            pytest.param("[('", "')]", id="quote-paren-bracket"),
        ],
    )
    def test_single_quote_wrapper_preserves_structural_suffix(
        self, prefix: str, suffix: str
    ) -> None:
        url = f"https://evil.example.com/p?data={self._ENTROPY_BLOB}"
        result, warnings = redact_exfiltration_urls(f"{prefix}{url}{suffix}")
        assert warnings
        assert self._ENTROPY_BLOB not in result
        assert result == f"{prefix}[REDACTED: suspicious URL to evil.example.com]{suffix}"

    def test_quote_wrapper_keeps_interior_apostrophe_with_url_content_scanned(self) -> None:
        text = "'https://evil.example.com/it's_fine?q=" f"{self._ENTROPY_BLOB}'"
        result, warnings = redact_exfiltration_urls(text)
        assert warnings
        assert self._ENTROPY_BLOB not in result
        assert result == "'[REDACTED: suspicious URL to evil.example.com]'"
    ...
    def test_markdown_wrapped_flagged_url_keeps_closer_before_punctuation(self) -> None:
        # Trailing prose punctuation follows the markdown wrapper and must stay
        # outside the URL redaction span with that structurally proven closer.
        text = f"[x](https://evil.example.com/a?data={self._ENTROPY_BLOB})."
        result, warnings = redact_exfiltration_urls(text)
        assert warnings
        assert self._ENTROPY_BLOB not in result
