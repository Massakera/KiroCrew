...
def test_activation_refusal_is_a_harness_seam_kas_reads_the_stamp_kiro_answers_none(caplog):
    ...
    harness refuses only a positive non-client stamp on the requested id, in
    plain words naming the remedy; the spawn-time hosts answer None for every
    response, so the Kiro path carries no branch."""
