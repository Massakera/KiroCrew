...
@pytest.mark.asyncio
async def test_worktree_state_names_the_trees_release_when_the_tip_is_unknown(monkeypatch):
    """A failed TIP lookup must not relabel a tagged checkout "not on a release".

    The tree's own release is a fact about the tree, read from the local mirror
    with no network call, so it is answered whether or not the channel tip
    resolved. The caller receives an empty ``resolved`` when ``resolve()``
    failed; the version still lands, ``at_tip`` stays False (no tip to compare).
    """
    ...
    monkeypatch.setattr(runtime, "_run_cmd", fake_run)
    got = await rcp.worktree_state("/wt", {})
    assert got["detached"] is True
    assert got["at_tip"] is False
    assert got["version"] == "0.5.0"
